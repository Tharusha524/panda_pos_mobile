 ...
 */
// Some cheap/clone thermal printers corrupt the leading byte(s) of a raw
// write — instead of consuming a command's control bytes silently, they
// print its identifier byte as a literal character (seen as a stray "2" or
// similar right before the first line). This isn't only the first-ever
// command of the whole job: it recurs at the start of *any* new native
// write, e.g. the write right after the logo's raster bands. So the warm-up
// byte has to be glued onto the front of the real payload's own write
// (below) rather than sent as its own separate transmission — otherwise the
// payload's first real command just becomes the new "first byte of a write"
// and takes the same hit the warm-up was meant to absorb. A lone 0x0A has no
// identifier byte to leak since it isn't an ESC-prefixed command, so
// whichever byte the glitch lands on, it's invisible.
...
const sendRawText = async (
  ...
  const sendOnce = async (): Promise<void> => {
    ...
    // Glue the warm-up byte onto the front of this same write — see comment
    // on WARM_UP_BYTE_BASE64 above for why it can't be its own transmission.
    base64 = bytesToBase64([...base64ToBytes(WARM_UP_BYTE_BASE64), ...base64ToBytes(base64)]);
...
export const bluetoothPrintService = {
  ...
  async printReport(
    ...
    currency?: string,
    settings?: PosMobileSettings | null,
    /** PNG (base64) of the on-screen report preview — see
     * receiptImageShare.captureReceiptBase64. Only used when the "print receipt as
     * image" setting is on; ignored (and safe to omit) otherwise. */
    capturedImageBase64?: string,
  ): Promise<void> {
  ...
  async printBackendReport(
     ...
     * image" setting is on; ignored (and safe to omit) otherwise. */
    capturedImageBase64?: string,
    /** Which catalog report this is — lets the text fallback below trim wide
     * reports down to their highest-value columns (see reportColumnFilters). */
  ...
  async printCustomerStatement(
    ...
    header: SystemReportHeader,
    settings?: PosMobileSettings | null,
    /** PNG (base64) of the on-screen customer statement preview — see
     * receiptImageShare.captureReceiptBase64. Only used when the "print receipt as
     * image" setting is on; ignored (and safe to omit) otherwise. */
    capturedImageBase64?: string,
  ): Promise<void> {
